...
def main_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="📦 Открыть пачку",
...
def collection_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="📊 По редкости",
                             callback_data="collection_by_rarity"),
        InlineKeyboardButton(text="🎌 По аниме",
                             callback_data="collection_by_anime"),
    )
    builder.row(
        InlineKeyboardButton(text="⭐ Избранные",
                             callback_data="collection_favorites"),
        InlineKeyboardButton(text="⚔️ В колоде",
                             callback_data="collection_in_deck"),
    )
    builder.row(
        InlineKeyboardButton(text="📈 Статистика",
                             callback_data="collection_stats"),
        InlineKeyboardButton(text="🔝 Самые\n сильные",
                             callback_data="collection_strongest"),
    )
    return builder.as_markup()
...
def rarity_keyboard() -> InlineKeyboardMarkup:
    ...
    for rarity, emj in zip(rarities, emoji):
        builder.add(
            InlineKeyboardButton(text=f"{emj} {rarity}",
...
def collection_keyboard(page: int,
                        has_next: bool,
                        rarity: str = None) -> InlineKeyboardMarkup:
    ...
    if page > 0:
        nav_row.append(
            InlineKeyboardButton(text="⬅️ Назад",
                                 callback_data=f"col_page:{page-1}:{rarity}"
                                 if rarity else f"col_page:{page-1}"))
    if has_next:
        nav_row.append(
...
def expedition_main_keyboard(active_count: int, uncollected_count: int,
                             slots: int,
                             free_slots: int) -> InlineKeyboardMarkup:
    ...
    if free_slots > 0:
        builder.row(
            InlineKeyboardButton(text="🕐 30 мин",
                                 callback_data="exped_new_short"),
            InlineKeyboardButton(text="🕑 2 часа",
                                 callback_data="exped_new_medium"),
        )
        builder.row(
            InlineKeyboardButton(text="🕕 6 часов",
                                 callback_data="exped_new_long"), )

    builder.row(
        InlineKeyboardButton(text=f"📋 Мои экспедиции ({active_count}/{slots})",
...
def expedition_cards_keyboard(cards: List[Tuple[UserCard, Card]],
                              selected_ids: List[int]) -> InlineKeyboardMarkup:
    ...
    for user_card, card in cards[:20]:  # Максимум 20 карт
        ...
        card_name = card.card_name[:20] + "..." if len(
            card.card_name) > 20 else card.card_name

        builder.row(
            InlineKeyboardButton(
                text=
                f"{status}{card_name} [{card.rarity}] Ур.{user_card.level}",
    ...
    builder.row(
        InlineKeyboardButton(
            text=
            "ℹ️ Можно выбрать 1-3 карты | Бонус за 2+ карт из 1 аниме +50%",
            callback_data="noop"))

    return builder.as_markup()


def expedition_confirm_keyboard(duration: str,
                                card_count: int) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="✅ Отправить в экспедицию",
                             callback_data=f"exped_start_{duration}"))

    builder.row(
        InlineKeyboardButton(text="◀️ Назад к выбору карт",
                             callback_data="exped_back_to_cards"),
        InlineKeyboardButton(text="❌ Отмена", callback_data="exped_cancel"))

    return builder.as_markup()


def expedition_list_keyboard(expeditions: List,
                             uncollected_count: int) -> InlineKeyboardMarkup:
    ...
    for exp in expeditions[:5]:  # Максимум 5
        ...
        builder.row(
            InlineKeyboardButton(text=f"{exp.name[:20]}... - {status}",
                                 callback_data=f"exped_info_{exp.id}"))

    if uncollected_count > 0:
        builder.row(
            InlineKeyboardButton(text=f"🎁 Забрать все ({uncollected_count})",
                                 callback_data="exped_claim_all"))
...
def card_detail_keyboard(card_id: int,
                         is_favorite: bool,
                         is_in_deck: bool,
                         can_upgrade: bool,
                         upgrade_cost: int = None,
                         user_dust: int = None) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text=favorite_text,
                             callback_data=f"favorite_{card_id}"),
    ...
    # Навигация
    builder.row(
        InlineKeyboardButton(text="« Назад в коллекцию",
                             callback_data="back_to_collection"))
...
def upgrade_card_keyboard(card_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="➕ Ещё +1",
                                 callback_data=f"upgrade_{card_id}"),
            InlineKeyboardButton(text="✖️ ×5",
                                 callback_data=f"5x_upgrade_{card_id}")
        ],
        [
