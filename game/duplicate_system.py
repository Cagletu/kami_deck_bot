...
async def check_for_duplicate(
    ...
    existing = await session.execute(
        select(UserCard).where(
            and_(UserCard.user_id == user_id, UserCard.card_id == card_id)
        )
    )
    ...
    return {"is_duplicate": False, "dust_earned": 0, "card": card}


async def process_duplicate(
    session: AsyncSession, user_id: int, card_id: int, dust_earned: int
) -> None:
