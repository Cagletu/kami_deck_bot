...
class ExpeditionManager:
    ...
    async def get_available_cards(
        ...
        cards.sort(key=lambda x: (rarity_order.get(x[1].rarity, 999), -x[0].level))
    ...
    async def get_active_expeditions(
        session: AsyncSession, user_id: int
    ) -> Tuple[List[Expedition], List[Expedition]]:
    ...
    async def calculate_rewards(
        session: AsyncSession, card_ids: List[int], duration_minutes: int
    ) -> dict:
        ...
        # Шанс на карту
        card_chance = min((duration_minutes // 60) * len(card_ids) * 20, 100)
    ...
    async def start_expedition(
        session: AsyncSession, user_id: int, card_ids: List[int], duration_type: str
    ) -> Expedition:
        """Начать экспедицию"""
        logger.info(
        ...
        duration_map = {"short": 30, "medium": 120, "long": 360}  # 30,  # 120,  # 360
